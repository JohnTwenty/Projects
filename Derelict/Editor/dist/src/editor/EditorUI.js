...
export class EditorUI {
    constructor(container, core, renderer) {
        this.container = container;
        this.core = core;
        this.renderer = renderer;
        this.segItems = new Map();
        container.innerHTML = layout;
        this.viewport = qs(container, '#viewport');
        this.overlay = qs(container, '#overlay');
        this.segPalette = qs(container, '#segment-palette');
        this.selectionList = qs(container, '#selection-list');
    ...
    wireEvents() {
        this.viewport.addEventListener('mousemove', (ev) => {
            ...
            const cell = pixelToCell({ x: ev.clientX - rect.left, y: ev.clientY - rect.top }, cellSize);
        ...
        const loadBtn = qs(this.container, '#btn-load');
        loadBtn.addEventListener('click', () => {
            this.openLoadDialog();
        });
        const saveBtn = qs(this.container, '#btn-save');
        saveBtn.addEventListener('click', () => {
            this.openSaveDialog();
        });
        const newBtn = qs(this.container, '#btn-new');
        ...
        const rotL = qs(this.container, '#rot-left');
        rotL.addEventListener('click', () => {
            this.core.rotate(-1);
            this.render();
            this.updateSelectionBar();
        });
        const rotR = qs(this.container, '#rot-right');
    ...
    async fetchMissionList() {
        const res = await fetch('missions/');
        const text = await res.text();
        const matches = [...text.matchAll(/href="([^"/]+\.txt)"/g)];
        return matches.map((m) => m[1]);
    }
    getLocalMissionNames() {
        if (typeof localStorage === 'undefined')
            return [];
        const names = [];
        for (let i = 0; i < localStorage.length; i++) {
            const k = localStorage.key(i);
            if (k && k.startsWith('mission:'))
                names.push(k.slice(8));
        }
        return names;
    }
    setPaletteSelection(segId) {
    ...
    updateSelectionBar() {
        ...
        if (contents.segment) {
            ...
            const li = this.makeSelectionItem('segment', contents.segment.instanceId, name);
            this.selectionList.appendChild(li);
        }
    ...
    makeSelectionItem(kind, id, label) {
        const li = createEl('li');
